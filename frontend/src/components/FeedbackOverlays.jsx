...
export const ConfirmActionModal = ({ confirmAction, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/45 backdrop-blur-md z-450 flex items-center justify-center p-4 animate-in fade-in duration-200" onClick={() => onClose(false)}>
      <div className="relative bg-white border border-slate-200 rounded-2xl shadow-xl w-full max-w-md p-5 animate-in zoom-in-95 duration-200" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="grid grid-cols-2 gap-3 mt-5">
          <button onClick={() => onClose(false)} className="py-2.5 bg-slate-50 text-slate-600 rounded-xl text-[13px] font-medium border border-slate-200 transition-all hover:bg-slate-100 active:translate-y-0">Cancelar</button>
          <button onClick={() => onClose(true)} className={`py-2.5 rounded-xl text-[13px] font-medium text-white transition-all active:translate-y-0 ${isDanger ? 'bg-rose-600' : 'bg-amber-600'}`}>
